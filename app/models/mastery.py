...
class MasteryLevel(BaseModel):
    ...
    forgetting_curve_params = Column(
        JSON, default=dict
    ...
    optimal_difficulty = Column(
        Float, default=0.5
    )  # Optimal difficulty for this user/concept
    challenge_level = Column(
        String(20), default="appropriate"
    )  # too_easy, appropriate, too_hard

    # Learning Path Context
    prerequisite_mastery_average = Column(
        Float, default=0.0
    )  # Average mastery of prerequisites
    dependent_concepts_unlocked = Column(
        Integer, default=0
    )  # How many concepts this unlocks
