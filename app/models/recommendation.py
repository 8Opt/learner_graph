...
class Recommendation(BaseModel):
    ...
    recommendation_type = Column(String(50), nullable=False)  # next_question, concept_review, streak_goal
    priority_score = Column(Float, nullable=False)  # 0.0 to 1.0, higher = more important
    ...
    algorithm_version = Column(String(50), nullable=False)  # Which algorithm generated this
    feature_weights = Column(JSON, default=dict)  # Feature importance for explainability
    
    # Context
    context_data = Column(JSON, default=dict)  # Additional context used in generation
    session_context = Column(String(100), nullable=True)  # morning, afternoon, weekend, etc.
    
    # Personalization Factors
    user_preferences_applied = Column(JSON, default=dict)  # Which preferences were used
    learning_style_match = Column(Float, nullable=True)  # How well this matches user's style
    
    # Status and Lifecycle
    status = Column(String(20), default="pending")  # pending, shown, accepted, rejected, expired
...
class ABTestExperiment(BaseModel):
    ...
    # Success Metrics
    primary_metric = Column(String(50), nullable=False)  # practice_minutes, retention_rate, etc.
    secondary_metrics = Column(JSON, default=list)
    target_improvement = Column(Float, nullable=False)  # Expected improvement percentage
